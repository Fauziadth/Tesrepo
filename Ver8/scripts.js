...
function test_define_rectangle() {
	...
	*/
    ...
    for (var i = 1; i<= 10; i++){
        ...
        p.m = 2;
        p.d = 0.2;
        p.r = new Vect3(Math.floor(Math.random()*100)/100, Math.floor(Math.random()*100)/100, 0.25);
        p.v = new Vect3(Math.floor(Math.random()*2)/100, Math.floor(Math.random()*2)/100, 0);
        pars.push(p);
    }
    */
    var p = new Sphere();
...
function test_button_create_button() {
	...
	var btn = document.createElement("button");
	btn.innerHTML = "Gaskuy";
	document.body.appendChild(btn);
	
	// Add an event only to to first button
	btn.addEventListener("click", test_define_rectangle);
